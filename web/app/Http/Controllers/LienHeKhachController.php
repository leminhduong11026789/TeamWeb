...
class LienHeKhachController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if(!empty($search)){
            $lienHeKhaches=$this->lienHeKhachRepository->findByField('ten_lien_he','LIKE','%'.$search['name'].'%',['*'],true,10);
        }
        else{
            $lienHeKhaches = $this->lienHeKhachRepository->paginate(10);
        }
    ...
    public function store(CreateLienHeKhachRequest $request)
    {
        ...
        Flash::success(__('messages.lienhe_saved'));
        if($input['save']==='save_edit'){
            return redirect(route('admin.lienHeKhaches.edit', $lienHeKhach->id));
        }
        elseif ($input['save']==='save_new'){
            return redirect(route('admin.lienHeKhaches.create'));
        }
        else{
    ...
    public function show($id)
    {
        ...
        if (empty($lienHeKhach)) {
            Flash::error(__('messages.lienhe_not_found'));
    ...
    public function edit($id)
    {
        ...
        if (empty($lienHeKhach)) {
            FFlash::error(__('messages.lienhe_not_found'));
    ...
    public function update($id, UpdateLienHeKhachRequest $request)
    {
        ...
        Flash::success(__('messages.lienhe_updated'));
     ...
     * @return Response
     */
    private function checkLienHe($ids){
        foreach ($ids as $id){
            $khach = $this->lienHeKhachRepository->findWithoutFail($id);
            if (empty($khach)) {
                return false;
            }
        }
    ...
    public function destroy($id, Request $request)
    {
        if($id=='MULTI'){
            if($request->ids!=null){
                ...
                if($this->checkLienHe($request->ids)){
                    $this->lienHeKhachRepository->destroy_multiple($request->ids);
                    Flash::success(__("messages.lienhe_deleted"));
                }
                else{
                    Flash::error(__("messages.lienhe_not_found"));
                }
