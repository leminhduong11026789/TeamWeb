...
abstract class BaseRepository extends \Prettus\Repository\Eloquent\BaseRepository
{
    ...
    public function buildTree($name, $columns = ['*'], $separator = '— ', $id = null){
        $categoriesParent = $this->model->whereRaw('parent_id = id')->get($columns);
        if ($id != null) {
            $categoriesParent = $this->model->whereRaw('parent_id = id')->where('id', '<>', $id)->get($columns);
        }
    ...
    public function getChild($name, $category, $level, $columns = ['*'], $separator = '— ', $id = null) {
        $level .= $separator;
        $sqlQuery = 'parent_id != id && parent_id = '.$category->id;
        $childrens =  $this->model->whereRaw($sqlQuery)->get($columns);
        foreach ($childrens as $children) {
            if ($children->id != $id || $id == null) {
                $children->$name = $level.$children->$name;
                $this->categoriesTree[] = $children;
                $this->getChild($name, $children, $level, $columns, $separator, $id);
            }
        }
    }
