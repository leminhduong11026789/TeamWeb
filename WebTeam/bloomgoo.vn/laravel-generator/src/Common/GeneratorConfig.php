...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('bloomgoo.laravel_generator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('bloomgoo.laravel_generator.namespace.model', 'App\Models').$prefix;
        if (config('bloomgoo.laravel_generator.ignore_model_prefix', false)) {
            $this->nsModel = config('bloomgoo.laravel_generator.namespace.model', 'App\Models');
        }
        $this->nsDataTables = config('bloomgoo.laravel_generator.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'bloomgoo.laravel_generator.model_extend_class',
            'Illuminate\Database\Eloquent\Model'
        );

        $this->nsApiController = config(
            'bloomgoo.laravel_generator.namespace.api_controller',
            'App\Http\Controllers\API'
        ).$prefix;
        $this->nsApiRequest = config('bloomgoo.laravel_generator.namespace.api_request', 'App\Http\Requests\API').$prefix;

        $this->nsRequest = config('bloomgoo.laravel_generator.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('bloomgoo.laravel_generator.namespace.request', 'App\Http\Requests');
        $this->nsBaseController = config('bloomgoo.laravel_generator.namespace.controller', 'App\Http\Controllers');
        $this->nsController = config('bloomgoo.laravel_generator.namespace.controller', 'App\Http\Controllers').$prefix;
    }
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'bloomgoo.laravel_generator.path.repository',
            app_path('Repositories/')
        ).$prefix;

        $this->pathModel = config('bloomgoo.laravel_generator.path.model', app_path('Models/')).$prefix;
        if (config('bloomgoo.laravel_generator.ignore_model_prefix', false)) {
            $this->pathModel = config('bloomgoo.laravel_generator.path.model', app_path('Models/'));
        }

        $this->pathDataTables = config('bloomgoo.laravel_generator.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
            'bloomgoo.laravel_generator.path.api_controller',
            app_path('Http/Controllers/API/')
        ).$prefix;

        $this->pathApiRequest = config(
            'bloomgoo.laravel_generator.path.api_request',
            app_path('Http/Requests/API/')
        ).$prefix;

        $this->pathApiRoutes = config('bloomgoo.laravel_generator.path.api_routes', app_path('Http/api_routes.php'));

        $this->pathApiTests = config('bloomgoo.laravel_generator.path.api_test', base_path('tests/'));

        $this->pathApiTestTraits = config('bloomgoo.laravel_generator.path.test_trait', base_path('tests/traits/'));

        $this->pathController = config(
            'bloomgoo.laravel_generator.path.controller',
            app_path('Http/Controllers/')
        ).$prefix;

        $this->pathRequest = config('bloomgoo.laravel_generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('bloomgoo.laravel_generator.path.routes', app_path('Http/routes.php'));

        $this->pathViews = config(
            'bloomgoo.laravel_generator.path.views',
            base_path('resources/views/')
        ).$viewPrefix.$this->mSnakePlural.'/';

        $this->modelJsPath = config(
                'bloomgoo.laravel_generator.path.modelsJs',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('bloomgoo.laravel_generator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('bloomgoo.laravel_generator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        $this->options['softDelete'] = config('bloomgoo.laravel_generator.options.softDelete', false);
    ...
    {
        $this->prefixes['route'] = explode('/', config('bloomgoo.laravel_generator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('bloomgoo.laravel_generator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('bloomgoo.laravel_generator.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('bloomgoo.laravel_generator.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('bloomgoo.laravel_generator.add_on.swagger', false);
        $this->addOns['tests'] = config('bloomgoo.laravel_generator.add_on.tests', false);
        $this->addOns['datatables'] = config('bloomgoo.laravel_generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('bloomgoo.laravel_generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('bloomgoo.laravel_generator.add_on.menu.menu_file', 'layouts.menu');
    }
}
